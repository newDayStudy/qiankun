...
export default {
  ...
  mounted () {
    this.result = this.handleLRC()
    this.$nextTick(() => {
      const audio = document.getElementById('audio')
      const li = document.querySelectorAll('.gc li')
      const height = Array.from(li)[0].offsetHeight
      console.log(height)
      audio.addEventListener('timeupdate', () => {
        const currentAudioTime = audio.currentTime
        console.log('currentAudioTime', currentAudioTime)
        const index = this.result.findIndex(item => item.time > currentAudioTime) - 1
        this.currentIndex = index < 0 ? this.result.length -1 : index
        this.translateY = -this.currentIndex * height
      })
      audio.addEventListener('timeend', () => {
        console.log('结束了')
        this.translateY = 0
      })
    })
  ...
  methods: {
    parseTime(lrcTime) {
      const lrcTimeArr = lrcTime.split(":");
      return parseInt(lrcTimeArr[0]) * 60 + parseInt(lrcTimeArr[1]);
    },
    handleLRC(){
      const lrcArr = document.querySelector('p').textContent.split('\n')
      let result = []
      for(let item of lrcArr){
        const lrcData = item.split('[');
        for(let k of lrcData){
          const lrc = k.split(']')
          if(lrc != ' '){
            const lrcTime = lrc[0]
            let obj = {
              time: this.parseTime(lrcTime.split('.')[0]),
              word: lrc[1]
            }
            result.push(obj);
          }
...
<template>
  <div>
    ...
    <p style="display: none;">
      [00:00.000] 作词 : 童子-T/Shingo.S
      [00:01.000] 作曲 : 童子-T
      [00:02.12]采样曲：願い~あの頃のキミへ~
      [00:12.15]中文填词：一只然
      [00:15.23]OP（原属词曲版权公司）：テレビ東京ミュージック 东京电视台音乐
